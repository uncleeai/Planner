...
-- ===== Zamknięta paczka =====
-- Apka jest prywatna: dane widzą wyłącznie e-maile z tej listy (plus admini
-- z is_admin()). Obce rejestracje i konta anonimowe odbijają się od RLS nawet
-- przy otwartych rejestracjach w Auth. E-mail z JWT jest weryfikowany przez
-- Supabase Auth, więc nie da się podszyć.
-- WAŻNE: trzymaj listę w synchronie z src/lib/members.ts (ekran odmowy w UI).
-- Po zmianie listy uruchom ten plik ponownie.
...
as $$
  select public.is_admin() or coalesce(
    lower(auth.jwt() ->> 'email') = any (array[
      'tomaszproblemx@gmail.com'
      -- ,'kolejna.osoba@example.com'  ← dopisz CAŁĄ paczkę przed uruchomieniem!
    ]),
    false
  );
$$;

-- Polityki RESTRICTIVE and-ują się z istniejącymi (permissive) — członkostwo
-- jest wymagane NA DODATEK do dotychczasowych reguł właścicielskich.
drop policy if exists "members only" on public.events;
create policy "members only" on public.events as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

drop policy if exists "members only" on public.slots;
create policy "members only" on public.slots as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

drop policy if exists "members only" on public.votes;
create policy "members only" on public.votes as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

drop policy if exists "members only" on public.profiles;
create policy "members only" on public.profiles as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

drop policy if exists "members only" on public.comments;
create policy "members only" on public.comments as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

drop policy if exists "members only" on public.push_subscriptions;
create policy "members only" on public.push_subscriptions as restrictive for all to authenticated
  using (public.is_member()) with check (public.is_member());

-- Storage: tylko bucket avatars (warunek nie dotyka innych bucketów);
-- publiczne odczyty po URL idą poza rolą authenticated, więc zdjęcia dalej działają.
drop policy if exists "members only avatars" on storage.objects;
create policy "members only avatars" on storage.objects as restrictive for all to authenticated
  using (bucket_id <> 'avatars' or public.is_member())
  with check (bucket_id <> 'avatars' or public.is_member());

...
└── src/
    ...
    │   ├── HapticSwitch.tsx      # Niewidoczny <input switch> na przycisku → tyk na iOS 26.5+ przy tapnięciu (głos, ping, LOCK IN, kadrowanie)
...
- **comments** — komentarze pod wypadem (koordynacja): `event_id` + `user_id` (konto)
  ...
  Wejście/wyjście „karta się rozkłada" (WAAPI): tło okna (.chat-ghost) rośnie z karty
  transformem — bez clip-path, który ucinał treść w locie — pole
...
- **comment_reactions** — reakcje emoji na komentarze (styl Messengera): PK
  ...
  UX: long-press komentarza jak w iMessage (`ReactionLayer`): kopia dymka na warstwie
  nad przygaszonym czatem, oryginał w liście znika; gdy pod nią brak miejsca na menu,
  podjeżdża do góry (+ lekko rośnie) — lista się nie rusza. Nad nią pasek reakcji
  (emotki po kolei), pod nią menu akcji (Kopiuj / Edytuj / Usuń); dotknięcie obok zamyka;
